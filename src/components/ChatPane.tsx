  ...
  return (
    <section className="flex min-h-[620px] flex-col bg-panel p-5" aria-labelledby="chat-heading">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto py-5">
        ...
        {messages.map((message) => <div key={message.id} className={message.role === "user" ? "ml-5" : "mr-3"}><div className={`border p-3 text-sm leading-6 ${message.role === "user" ? "border-brand/40 bg-brand/10 text-foreground" : message.role === "system" ? "border-danger/50 bg-danger/10 text-danger" : "border-border bg-panel-raised text-muted"}`}><p className="mb-2 font-mono text-[10px] uppercase tracking-[0.1em] text-brand">{message.role}</p>{message.content}</div></div>)}
